...
func (lc *LoRaConfig) Validate() error {
	...
	if err := lc.validateDevice(); err != nil {
		return err
	}

	if err := lc.validateMeshKey(); err != nil {
		return err
	}

	if err := lc.validateRadioParameters(); err != nil {
		return err
	}
...
// validateDevice checks device path or UDP configuration.
func (lc *LoRaConfig) validateDevice() error {
	if lc.Device == "udp" {
		if lc.UDPListen == "" {
...
// validateMeshKey checks the mesh encryption key format and length.
func (lc *LoRaConfig) validateMeshKey() error {
	if lc.MeshKey == "" {
...
// validateRadioParameters checks frequency, spreading factor, and bandwidth.
func (lc *LoRaConfig) validateRadioParameters() error {
