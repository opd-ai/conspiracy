...
// mockAdminAPI creates a mock cjdns admin API server.
...
func newMockAdminAPI(password string) *mockAdminAPI {
	mock := &mockAdminAPI{
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if mock.password != "" {
			if pwd, ok := req["password"].(string); !ok || pwd != mock.password {
		...
		case "UDPInterface_beginConnection":
			...
			mock.addedPeers = append(mock.addedPeers, addr)
			json.NewEncoder(w).Encode(map[string]interface{}{
				"result": "success",
			})
		case "ping":
...
func TestNewConsumer(t *testing.T) {
	...
	if c.config.Password != cfg.Password {
		t.Errorf("expected password %s, got %s", cfg.Password, c.config.Password)
	}
...
func TestConsume_RouteRemoved(t *testing.T) {
	...
	// cjdns doesn't actively remove peers, so no API call expected
	// Just verify the peer was removed from cache
	c.mu.RLock()
	_, exists := c.peerCache[12345]
	c.mu.RUnlock()

	if exists {
		t.Error("peer should have been removed from cache")
	}
}
...
func TestConsume_WithPassword(t *testing.T) {
	password := "secret123"
	mock := newMockAdminAPI(password)
	defer mock.Close()
	...
	err := c.Consume(h)
	if err != nil {
		t.Fatalf("Consume failed with correct password: %v", err)
	}
...
func TestConsume_WrongPassword(t *testing.T) {
	password := "secret123"
	mock := newMockAdminAPI(password)
	defer mock.Close()
...
func TestExtractCjdnsAddress_TCP(t *testing.T) {
	c := NewConsumer(AdminAPIConfig{Address: "127.0.0.1:11234"})

	addr, _ := net.ResolveTCPAddr("tcp", "192.168.1.100:9001")
	result, err := c.extractCjdnsAddress(addr)
	if err != nil {
		t.Fatalf("extractCjdnsAddress failed: %v", err)
	}
...
func TestExtractCjdnsAddress_UDP(t *testing.T) {
	c := NewConsumer(AdminAPIConfig{Address: "127.0.0.1:11234"})
...
func TestExtractCjdnsAddress_Nil(t *testing.T) {
	c := NewConsumer(AdminAPIConfig{Address: "127.0.0.1:11234"})

	_, err := c.extractCjdnsAddress(nil)
	if err == nil {
